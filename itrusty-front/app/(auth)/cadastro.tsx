    ...
    try {
      if (Platform.OS === 'web') {
        ...
        if (result.type === 'success') {
          ...
          if (session) {
            const res = await api.post<AuthResponse>('/auth/social', {
    ...
    <KeyboardAvoidingView style={s.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.7}
        >
          <View style={[s.checkbox, termos && s.checkboxMarcado]}>
            {termos && <Text style={s.checkMarca}>✓</Text>}
          </View>
          <Text style={s.checkTexto}>Li e aceito os <Text style={s.checkLink}>termos de uso</Text></Text>
        </TouchableOpacity>
        {erros.termos && <Text style={s.erro}>{erros.termos}</Text>}

        {erros.api && <Text style={[s.erro, s.erroApi]}>{erros.api}</Text>}

        <TouchableOpacity style={[s.botao, loading && s.botaoDisabled]} onPress={handleCadastro} disabled={loading}>
          <Text style={s.botaoTexto}>{loading ? 'Criando conta...' : 'Criar conta →'}</Text>
        </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  checkRow:          { flexDirection: 'row', alignItems: 'center', marginTop: 8, marginBottom: 4 },
  checkbox:          { width: 20, height: 20, borderRadius: 6, borderWidth: 1.5, borderColor: Colors.border, marginRight: 10, justifyContent: 'center', alignItems: 'center' },
  checkboxMarcado:   { backgroundColor: Colors.accent, borderColor: Colors.accent },
  checkMarca:        { color: Colors.white, fontSize: 12, fontWeight: '700' },
  checkTexto:        { fontSize: 13, color: Colors.textLight },
